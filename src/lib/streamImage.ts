...
export async function streamImage(
  ...
  if (!res.ok || !res.body) {
    ...
    if (res.status === 503) throw new Error("Image generation is not connected yet");
    throw new Error(detail.slice(0, 200) || `Image generation failed (${res.status})`);
  }
  ...
  if (!sawAnyEvent) {
    ...
    if (!replay.ok) {
      throw new Error(
        (await replay.text().catch(() => "")).slice(0, 200) ||
          `Image generation failed (${replay.status})`,
      );
    }
  ...
  if (!sawCompleted) throw new Error("Image generation stopped before finishing");
}
